        ...
        if (dialogMode == MODE_NEW) {
            builder.setPositiveButton(R.string.add_button, new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialog, int id) {
                    ...
                    MFGeofence fence = new MFGeofence(UUID.randomUUID().toString(), name, desc, position.latitude, position.longitude, radius);
                    mapsActivity.customHttpService.addGeofence(fence, new HttpRequestCallback<MFGeofence>() {
                        @Override
                        public void onSuccess(MFGeofence fence) {
                            if (fence != null) {
            ...
            builder.setPositiveButton(R.string.update_button, new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialog, int id) {
                    fence = new MFGeofence(fence.getCode(), nameView.getText().toString(), descView.getText().toString(),
                        position.latitude, position.longitude, radius);
                    mapsActivity.customHttpService.updateGeofence(fence, new HttpRequestCallback<MFGeofence>() {
                        @Override
                        public void onSuccess(MFGeofence fence) {
                            if (fence != null) {
            ...
            builder.setNeutralButton(R.string.delete_button, new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialog, int id) {
                    mapsActivity.customHttpService.removeGeofence(fence, new HttpRequestCallback<MFGeofence>() {
                        @Override
                        public void onSuccess(MFGeofence fence) {
