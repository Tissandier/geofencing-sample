 ...
 */
public class MFGeofencingManager {
    /**
     * Logger for this class.
     */
    private static final Logger log = LoggingConfiguration.getLogger(MFGeofencingManager.class.getSimpleName());
    static final String INTENT_ID = "IBMGeofencingService";
    /**
     * Part of a request path pointing to the geofence connector.
     */
    static final String GEOFENCE_CONNECTOR_PATH = "events";
    /**
     ...
     */
    public MFGeofencingManager(Context context, String baseURL, String username, String password, int maxDistance) {
        this(null, MODE_APP, context, baseURL, username, password, maxDistance);
    }
     ...
     * Defines the bounding box for the monitored geofences: square box with a {@code maxDistance} side centered on the current location.
     */
    MFGeofencingManager(Settings settings, int mode, Context context, String baseURL, String username, String password, int maxDistance) {
        log.debug("mf-geofence version " + BuildConfig.VERSION_NAME);
        this.mMode = mode;
        this.mMaxDistance = maxDistance;
        this.mHttpService = new HttpService(baseURL, username, password);
        this.mContext = context;
        this.mSettings = (settings != null) ? settings : new Settings(context);
        log.debug("MFGeofencingManager() settings = " + this.mSettings);
    ...
    /**
     * Send a notification to the backend as an HTTP request.
        ...
        AsyncTask<Void, Void, Void> task = new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... params) {
                ...
                try {
                    ...
                    log.debug(String.format(Locale.US, "loaded %,d geofences from resource '[%s]'", allGeofences.size(), resource));
                } catch(Exception e) {
            ...
            protected void onPostExecute(Void aVoid) {
                if (error != null) {
                    ...
                    try {
                        ...
                        MFGeofenceEvent.toIntent(broadcastIntent, MFGeofenceEvent.Type.SERVER_SYNC, geofenceList.getGeofences(), null);
    ...
    private void loadGeofencesFromServer(long lastSyncTimestamp) {
        HttpRequestCallback<JSONObject> cb = new HttpRequestCallback<JSONObject>() {
            @Override
            public void onSuccess(JSONObject result) {
                try {
                    ...
                    if (!geofences.isEmpty() || !list.getDeletedGeofenceCodes().isEmpty()) {
                        Intent broadcastIntent = new Intent(MFGeofenceEvent.ACTION_GEOFENCE_EVENT);
                        broadcastIntent.setPackage(mContext.getPackageName());
                        MFGeofenceEvent.toIntent(broadcastIntent, MFGeofenceEvent.Type.SERVER_SYNC, geofences, list.getDeletedGeofenceCodes());
                        mContext.sendBroadcast(broadcastIntent);
                    }
                    log.debug("loadGeofences() got " + list.getGeofences().size() + " geofences");
                } catch (Exception e) {
                    HttpRequestError error = new HttpRequestError(-1, e, "error while parsing JSON");
        ...
        JSONPayloadRequest request = new JSONPayloadRequest(cb, HttpMethod.GET, null);
        request.setPath(String.format("%s", CONFIG_CONNECTOR_PATH));
    ...
    private void setInitialLocation() {
        Runnable r = new Runnable() {
            ...
            public void run() {
                ...
                if (last != null) {
                    new LocationUpdateReceiver(MFGeofencingManager.this).onLocationChanged(last, true);
                }
            }
        };
        new Thread(r).start();
    ...
    /**
     * Retrieve the last usFloced device descritpor, if any. If none exists, one is created from the {@link GeofencingDeviceInfo} API.
     * @return the device descriptor.
     */
    String retrieveDeviceDescriptor() {
        String result = mSettings.getString(GeofencingDeviceInfo.DESCRIPTOR_KEY, null);
        if (result == null) {
            SharedPreferences prefs = mContext.getSharedPreferences(GeofencingDeviceInfo.SHARED_PREF, Context.MODE_PRIVATE);
