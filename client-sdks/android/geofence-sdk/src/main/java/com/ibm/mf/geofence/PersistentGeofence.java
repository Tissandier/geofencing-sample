...
/**
 * Instances of this class represent geofences that are persisted to a local database.
 */
public class PersistentGeofence extends SugarRecord implements Serializable {
    ...
    MFGeofence toMFGeofence() {
        return new MFGeofence(mCode, mName, mDescription, mLatitude, mLongitude, mRadius);
    }

    static PersistentGeofence fromMFGeofence(MFGeofence piGeofence) {
    ...
    static List<MFGeofence> toMFGeofences(List<PersistentGeofence> list) {
        List<MFGeofence> result = new ArrayList<>(list.size());
        for (PersistentGeofence pg: list) {
            result.add(pg.toMFGeofence());
        }
        return result;
    }
}
