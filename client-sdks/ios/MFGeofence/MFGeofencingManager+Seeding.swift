...
extension MFGeofencingManager {
	...
	*/
	...
	public func seedGeojsonWithURL(
		url:NSURL,
		propertiesGenerator:MFGeofencePropertiesGenerator? = nil,
	...
	*/
	...
	func seedGeojson(moc:NSManagedObjectContext,geojson:[String:AnyObject],
		propertiesGenerator:MFGeofencePropertiesGenerator? = nil) -> MFGeofencingError? {

			guard let type = geojson["type"] as? String else {
				return MFGeofencingError.GeoJsonMissingType
			}

			guard type == "FeatureCollection" else {
				DDLogError("PIGeofencingError.GeoJsonWrongType \(type)")
				return MFGeofencingError.GeoJsonWrongType
			}
			...
			guard var geofences = geojson["features"] as? [[String:AnyObject]] else {
				DDLogError("MFGeofencingError.GeoJsonNoFeature")
				return MFGeofencingError.GeoJsonNoFeature
			}
			...
			do {
				...
				if let deleted = deleted {
					...
					var deletedFences:[MFGeofence]?
					repeat {
						autoreleasepool {
							do {
								let deletedRequest = MFGeofence.fetchRequest
								deletedRequest.fetchLimit = 100
								deletedRequest.predicate = NSPredicate(format: "code in %@",deletedSet)
								deletedFences = try moc.executeFetchRequest(deletedRequest) as? [MFGeofence]
				...
				MFGeofencePreferences.lastSyncDate = lastSyncDate
				...
				if nbErrors == 0 {
					...
					DDLogError("PIGeofencingError.WrongFences \(nbErrors)")
					return MFGeofencingError.WrongFences
				}
			} catch {
				DDLogError("Core Data Error \(error)")
				assertionFailure("Core Data Error \(error)")
				return MFGeofencingError.InternalError
			}
	}
