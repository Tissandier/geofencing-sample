...
public let MFServiceError = "com.ibm.MF.Error"

public typealias MFResult = HTTPOperationResult

public protocol MFServiceDelegate:class {
...
public final class MFService: NSObject {
	...
	public weak var delegate:MFServiceDelegate?
    ...
    public init(baseURL:String, username:String, password:String){
        ...
        httpQueue.name = "com.ibm.geofence.service-queue"
    ...
    public func executeRequest(request:MFRequest) -> MFResponse {
	...
	public func executeDownload(request:MFDownloadRequest) -> MFDownloadResponse? {
	...
	lazy var backgroundServiceSession:NSURLSession = {
		...
		let identifier = "com.ibm.MF." + NSUUID().UUIDString
		...
		session.sessionDescription = "com.ibm.MF \(NSDate().description)"
...
extension MFService : NSURLSessionDelegate {
    
    
    public func URLSession(session: NSURLSession, task: NSURLSessionTask, didReceiveChallenge challenge: NSURLAuthenticationChallenge, completionHandler: (NSURLSessionAuthChallengeDisposition, NSURLCredential?) -> Void) {

		DDLogVerbose("MFService.didReceiveChallenge for task \(task.taskIdentifier) \(task.taskDescription ?? "")",asynchronous:false)
    ...
    public func URLSession(session: NSURLSession, didReceiveChallenge challenge: NSURLAuthenticationChallenge, completionHandler: (NSURLSessionAuthChallengeDisposition, NSURLCredential?) -> Void) {
		...
		DDLogVerbose("MFService.didReceiveChallenge for session description \(session.sessionDescription ?? "No Desc")",asynchronous:false)
	...
	public func URLSessionDidFinishEventsForBackgroundURLSession(session: NSURLSession) {
		DDLogInfo("MFService.URLSessionDidFinishEventsForBackgroundURLSession",asynchronous:false)
	...
	public func URLSession(session: NSURLSession, didBecomeInvalidWithError error: NSError?) {
		DDLogError("MFService.didBecomeInvalidWithError session description \(session.sessionDescription ?? "No session description") , error \(error)",asynchronous:false)
	}
}

extension MFService : NSURLSessionDownloadDelegate {
	...
	public func URLSession(session: NSURLSession, downloadTask: NSURLSessionDownloadTask, didFinishDownloadingToURL location: NSURL) {
		DDLogError("MFService.didFinishDownloadingToURL session description \(session.sessionDescription ?? "No session description") , task identifier \(downloadTask.taskIdentifier) , task description \(downloadTask.taskDescription ?? "No task description")",asynchronous:false)
		let libraryURL = GeofenceUtils.libraryDirectory
	...
	public func URLSession(session: NSURLSession, task: NSURLSessionTask, didCompleteWithError error: NSError?) {
		if let error = error {
			DDLogError("MFService.didCompleteWithError session description \(session.sessionDescription ?? "No session description") , task identifier \(task.taskIdentifier) , task description \(task.taskDescription ?? "No task description") , error \(error)",asynchronous:false)
		}
