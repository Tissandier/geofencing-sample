...
extension MFGeofencingManager {
	...
	*/
	...
	func compareGeofence(currentPosition:CLLocation) -> (a:MFGeofence,b:MFGeofence) -> Bool {
		return { (a:MFGeofence,b:MFGeofence) -> Bool in
		...
		*/
		...
		do {
			if self.regions == nil {
				...
				let fetchMonitoredRegionsRequest = MFGeofence.fetchRequest

				fetchMonitoredRegionsRequest.predicate = NSPredicate(format: "monitored == true")
				guard let monitoredGeofences = try moc.executeFetchRequest(fetchMonitoredRegionsRequest) as? [MFGeofence] else {
			...
			for (geofenceCode,region) in self.regions! {
				if geofenceCodes.contains(geofenceCode) == false {
					...
					let geofences = try moc.executeFetchRequest(fetchRequest) as? [MFGeofence]
