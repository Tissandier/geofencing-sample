...
public final class MFGeofenceCreateRequest:NSObject,MFRequest {
    
    public let completionBlock: MFGeofenceCreateResponse -> Void
    ...
    public init(geofenceName:String,geofenceDescription:String?,geofenceRadius:Int,geofenceCoordinate:CLLocationCoordinate2D,completionBlock:MFGeofenceCreateResponse -> Void) {
    ...
    /// - param service: The MF service
    /// Execute this request against the MF service
    public func execute(service:MFService) -> MFResponse {
        
        let operation = MFGeofenceCreateOperation(
            service:service,
        ...
        let response = MFGeofenceCreateResponse(mfRequest: self,operation:operation)
                ...
                case .OK(let data)?:
                    ...
                    do {
                        ...
                        guard let properties = json["properties"] as? [String:AnyObject] else {
                            DDLogError("MFGeofenceCreateResponse,Missing properties",asynchronous:false)
                            break
                        }
                        
                        guard let geofenceCode = properties["@code"] as? String else {
                            DDLogError("MFGeofenceCreateResponse,Missing Fence ID",asynchronous:false)
                            break
                        }
                        
                        response.geofenceCode = geofenceCode
                    } catch {
                        DDLogError("MFGeofenceCreateResponse,Json parsing error \(error)",asynchronous:false)
                        response.result = .OK(nil)
                    }
                case .Cancelled?:
